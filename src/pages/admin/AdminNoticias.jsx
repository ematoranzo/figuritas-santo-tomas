...
export default function AdminNoticias() {
  ...
  async function cargarNoticias() {
    let q = supabase
      .from('noticia')
      .select(
        '*, alumno:id_alumno_ref(nombre, apellido), album:id_album_ref(nombre)'
      )
  ...
  async function handleSubmit(e) {
    ...
    try {
      const datos = {
        ...
        visible_sin_login: form.visible_sin_login,
        estado: form.estado,
        fecha_publicacion:
          form.estado === 'publicada' ? new Date().toISOString() : null,
      };
      if (editando) {
  ...
  async function cambiarEstado(id, nuevoEstado) {
    ...
    if (nuevoEstado === 'publicada')
      datos.fecha_publicacion = new Date().toISOString();
  ...
  function editar(n) {
    setForm({
      ...
      visible_sin_login: n.visible_sin_login || false,
      estado: n.estado,
    });
    setEditando(n.id);
    setMostrarForm(true);
          ...
          <form onSubmit={handleSubmit} className="admin-form">
            ...
            >
              ...
              >
                <input
                  type="checkbox"
                  name="visible_sin_login"
                  checked={form.visible_sin_login}
                  onChange={handleChange}
                />
                <span>👁 Visible sin login</span>
              </label>
            </div>
          ...
          {noticias.map((n) => (
            <div key={n.id} className="noticia-admin-card">
                ...
                <div className="noticia-admin-info">
                  ...
                  >
                    <span
                      ...
                      style={{ background: COLORES[n.estado] }}
                    >
                      {n.estado}
                    </span>
                    {n.destacada && (
                  ...
                  {n.resumen && (
                    <p
                      style={{ color: '#666', fontSize: '.9rem', marginTop: 6 }}
                    >
                      {n.resumen}
                    </p>
                  )}
              ...
              <div className="acciones">
                {n.estado === 'borrador' && (
                  <button
                    onClick={() => cambiarEstado(n.id, 'publicada')}
                ...
                {n.estado === 'publicada' && (
                  <button
                    onClick={() => cambiarEstado(n.id, 'despublicada')}
